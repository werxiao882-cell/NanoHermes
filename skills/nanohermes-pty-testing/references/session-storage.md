...
| S-R01 | 标题搜索模糊匹配 | 搜索标题中的中文关键词 | 返回 LIKE 匹配结果 | [PTY] |
| S-R02 | 标题搜索无结果 | 搜索不存在的关键词 | 返回空列表 | [PTY] |

### CLI 恢复 (6)
| ID | 测试内容 | 操作步骤 | 预期 | 标记 |
|----|---------|---------|------|------|
| S-05 | 恢复最近会话 | `--resume` 启动（无参数） | 恢复最近结束的会话 | [PTY] |
| S-R03 | 按 ID 恢复 | `--resume <session_id>` 启动 | 按 ID 精确恢复会话 | [PTY] |
| S-R04 | 按标题恢复 | `--resume-title "关键词"` 启动 | 模糊匹配标题，恢复最佳匹配 | [PTY] |
| S-R05 | 恢复不存在的 ID | `--resume fake-id-123` | 提示"未找到会话" | [PTY] |
| S-R06 | 恢复不存在的标题 | `--resume-title "不存在的标题"` | 提示"未找到标题匹配" | [PTY] |
| S-R07 | 无历史会话时恢复 | 清空会话后 `--resume` | 提示"暂无历史会话可恢复" | [PTY] |

### TUI 命令恢复 (2)
| ID | 测试内容 | 操作步骤 | 预期 | 标记 |
|----|---------|---------|------|------|
| S-R08 | TUI 内 /resume | `/resume <session_id>` | 切换到指定会话继续对话 | [PTY] |
| S-R09 | TUI /resume 无参数 | `/resume`（无 ID） | 显示用法提示 | [PTY] |

### 恢复后验证 (2)
| ID | 测试内容 | 操作步骤 | 预期 | 标记 |
|----|---------|---------|------|------|
| S-R10 | 恢复后消息完整 | 恢复后发送"刚才我们说了什么" | AI 能引用之前对话内容 | [PTY] |
| S-R11 | 恢复后继续对话 | 恢复后继续工具链操作 | 新消息追加到原 JSONL 文件 | [PTY] |
