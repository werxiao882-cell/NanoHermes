...
class TestBuildReviewPrompt:
    ...
    def test_build_memory_review_prompt(self):
        ...
        assert "记忆" in prompt or "memory" in prompt.lower()
...
class TestReviewConstants:
    ...
    def test_memory_review_prompt_exists(self):
        ...
        assert MEMORY_REVIEW_PROMPT is not None
        assert len(MEMORY_REVIEW_PROMPT) > 0

    def test_skill_review_prompt_exists(self):
        """Test that skill review prompt is defined."""
        assert SKILL_REVIEW_PROMPT is not None
        assert len(SKILL_REVIEW_PROMPT) > 0
