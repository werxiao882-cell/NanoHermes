...
- [x] 1.1 创建 `src/session/` 目录结构
- [x] 1.2 配置 pytest 测试框架
...
- [x] 2.8 编写 SessionDB 初始化和 WAL 模式的单元测试
  - [x] 2.8.1 测试创建新数据库
  - [x] 2.8.2 测试打开已存在的数据库
  - [x] 2.8.3 测试 WAL 模式设置
  - [x] 2.8.4 测试 WAL 回退到 DELETE 模式
  - [x] 2.8.5 测试外键约束启用
  - [x] 2.8.6 测试 _execute_write 成功写入无竞争
  - [x] 2.8.7 测试锁竞争重试成功
  - [x] 2.8.8 测试达到最大重试次数
  - [x] 2.8.9 测试非锁定错误立即传播
  - [x] 2.8.10 测试定期 checkpoint
  - [x] 2.8.11 测试 checkpoint 失败不抛出异常
  - [x] 2.8.12 测试正常关闭
  - [x] 2.8.13 测试重复关闭
...
- [x] 3.5 编写 FTS 搜索的单元测试，包含 CJK 子串搜索
  ...
  - [x] 3.5.2 测试创建 trigram FTS5 表
  - [x] 3.5.3 测试插入消息同步到 FTS
  - [x] 3.5.4 测试删除消息同步到 FTS
  - [x] 3.5.5 测试更新消息同步到 FTS
  - [x] 3.5.6 测试搜索所有会话
  - [x] 3.5.7 测试按会话过滤搜索
  - [x] 3.5.8 测试搜索无结果
  - [x] 3.5.9 测试中文子串搜索
...
- [x] 4.5 编写 schema 协调的单元测试
  - [x] 4.5.1 测试解析单表列
  - [x] 4.5.2 测试解析多表列
  - [x] 4.5.3 测试解析带 DEFAULT 的列
  - [x] 4.5.4 测试添加缺失列
  - [x] 4.5.5 测试不添加已存在的列
  - [x] 4.5.6 测试处理重复列错误
  - [x] 4.5.7 测试协调后创建索引
  - [x] 4.5.8 测试索引创建失败不阻塞启动
  - [x] 4.5.9 测试初始化 schema_version
  - [x] 4.5.10 测试更新 schema_version

## 5. 会话生命周期管理

- [x] 5.1 实现 create_session 方法
- [x] 5.2 实现 end_session 方法，包含 ended_at 和 end_reason 设置
- [x] 5.3 实现 reopen_session 方法，清除 ended_at 和 end_reason
- [x] 5.4 实现 update_system_prompt 方法
- [x] 5.5 实现 update_token_counts 方法，支持增量和绝对模式
- [x] 5.6 实现 get_session 方法
- [x] 5.7 实现 get_compression_tip 方法，walk 压缩延续链
- [x] 5.8 编写会话生命周期的单元测试
  - [x] 5.8.1 测试创建新会话
  - [x] 5.8.2 测试幂等创建
  - [x] 5.8.3 测试创建会话带完整参数
  - [x] 5.8.4 测试结束会话
  - [x] 5.8.5 测试已结束会话不重复结束
  - [x] 5.8.6 测试恢复会话
  - [x] 5.8.7 测试创建压缩延续会话
  - [x] 5.8.8 测试获取压缩延续 tip
  - [x] 5.8.9 测试排除委托子节点
  - [x] 5.8.10 测试增量更新 token 计数
  - [x] 5.8.11 测试绝对更新 token 计数
  - [x] 5.8.12 测试更新模型信息
  - [x] 5.8.13 测试获取存在的会话
  - [x] 5.8.14 测试获取不存在的会话
...
- [x] 6.1 创建唯一标题索引 idx_sessions_title_unique
- [x] 6.2 实现 sanitize_title 方法，包含控制字符剥离和长度验证
- [x] 6.3 实现 set_session_title 方法，包含唯一性检查
- [x] 6.4 实现 get_session_title 方法
- [x] 6.5 实现 resolve_session_by_title 方法，支持精确匹配和编号变体
- [x] 6.6 实现 get_next_title_in_lineage 方法
- [x] 6.7 编写标题管理的单元测试
  - [x] 6.7.1 测试设置新标题
  - [x] 6.7.2 测试标题唯一性检查
  - [x] 6.7.3 测试更新自己的标题
...
- [x] 9.1 实现 --resume <session_id> 命令行参数
- [x] 9.2 实现 --resume-title "title" 命令行参数
- [x] 9.3 实现 --resume（无参数）恢复最近会话
- [x] 9.4 实现恢复时加载 JSONL 历史并显示摘要
- [x] 9.5 实现恢复失败时的错误处理和新建会话
- [x] 9.6 编写会话恢复的集成测试

## 10. 异步标题生成

- [x] 10.1 实现 generate_auto_title 异步调用（不阻塞对话）
- [x] 10.2 实现标题生成失败时的回退逻辑（截取用户首条消息）
- [x] 10.3 更新 main.py 使用异步标题生成
- [x] 10.4 编写异步标题生成的单元测试
