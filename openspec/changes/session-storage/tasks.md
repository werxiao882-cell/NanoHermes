...
- [ ] 7.1 编写 SessionDB 完整生命周期的集成测试
- [ ] 7.2 编写并发写入的集成测试，验证抖动重试
- [ ] 7.3 编写 FTS 搜索的集成测试，包含多会话搜索
- [ ] 7.4 编写 schema 协调的集成测试，验证列添加
- [ ] 7.5 编写完整工作流集成测试（创建会话 → 插入消息 → 搜索 → 结束 → 恢复）
- [ ] 7.6 编写压缩延续链集成测试
- [ ] 7.7 编写标题 lineage 集成测试
