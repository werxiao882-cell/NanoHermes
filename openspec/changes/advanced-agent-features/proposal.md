...
- 实现 **Hooks 系统**：基于 EventBus 责任链拦截机制，全部 18 种事件支持拦截器（修改数据/阻断流程/middleware 前后置），事件按 block 语义分三类（可阻断 3/可修改 7/仅观察 8），内置危险命令拦截 Hook，支持 Python callable 和 subprocess 脚本，完全向后兼容现有 21 处 emit 调用和 21 处 on 订阅
...
- `hooks-system`: EventBus 责任链拦截系统。拦截器签名 (data, next)→None，支持优先级排序和洋葱模型前后置。事件分三类：可阻断（MODEL_REQUEST/TOOL_START/ITERATION_END）、可修改（MODEL_RESPONSE/TOOL_END/MESSAGE_APPEND/ITERATION_START/PRE_COMPRESS）、仅观察（其余 8 种）。拦截器阻断后观察者仍触发。内置 dangerous_command_guard 复用 terminal.py DANGEROUS_PATTERNS 拦截危险命令。ScriptHook 支持外部脚本（subprocess + JSON stdin/stdout + 30s 超时）。拦截器仅作用于注册的 EventBus 实例，子 Agent 独立总线不受父级影响
...
- 新增 `src/hooks/` 目录：dangerous_command_guard.py、ScriptHook 包装类、配置加载器
- 修改 `src/conversation/events.py`：EventBus 增加责任链拦截（ChainResult + intercept + emit 改造）
- 修改 `src/conversation/loop.py`：17 处 emit 调用适配（3 处检查 blocked + 7 处读回 data）
- 验证 `src/delegation/manager.py`：3 处 emit 调用兼容性（无需改动）
- 修改 `tests/conversation/test_events.py`：修复 EventType 计数 + 适配 ChainResult
- 修改 `src/conversation/ARCHITECTURE.md`：记录拦截器机制
