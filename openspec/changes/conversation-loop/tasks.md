...
- [ ] 5.1 添加 rich 和 prompt_toolkit 依赖到 pyproject.toml
- [ ] 5.2 实现 TUI 布局类（顶部横幅、对话区域、输入区）
- [ ] 5.3 实现顶部横幅组件（模型、工具、技能、会话信息）
- [ ] 5.4 实现对话输出区域（流式显示工具调用和响应）
- [ ] 5.5 实现底部固定输入区
- [ ] 5.6 实现斜杠命令自动补全
- [ ] 5.7 实现 --tui 命令行参数启动 TUI 模式
- [ ] 5.8 编写 TUI 组件单元测试
