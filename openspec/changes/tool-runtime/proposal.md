...
- `default-tools`: 默认工具集，包含 clarify（向用户提问）、execute_code（运行 Python 脚本）、cronjob（定时任务）、delegate_task（子 Agent 委托）、memory（持久记忆）、session_search（历史会话搜索）、skills（技能管理）、process（后台进程管理）。
