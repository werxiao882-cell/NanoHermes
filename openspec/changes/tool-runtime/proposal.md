...
- 实现默认工具集，每个工具独立文件：
  - clarify_tools.py：向用户提问，支持预设选项和自定义输入
  - code_execution_tools.py：运行 Python 脚本
  - cronjob_tools.py：管理定时任务
  - delegation_tools.py：子 Agent 委托
  - memory_tools.py：持久记忆
  - session_search_tools.py：历史会话搜索
  - skills_tools.py：技能管理（skill_manage、skill_view、skills_list）
