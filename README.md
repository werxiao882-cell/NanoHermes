...
# 或手动安装
pip install openai anthropic pyyaml pydantic python-dotenv
```
...
│   │   ├── dispatcher.py             # 工具分发器 (错误包装为 JSON)
...
| **tool-runtime** | 工具注册表、工具集、分发器、终端工具、异步桥接 | ✅ 完成 |
| **session-storage** | SQLite 会话存储、FTS5 搜索、生命周期管理 | ✅ 完成 |
...
| **conversation-loop** | 核心对话循环、错误分类 | ✅ 完成 |
...
# 运行单个测试
python -m pytest tests/provider/test_credentials.py -v
```
